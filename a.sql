...
db.Filmes.find({}, { Nome: 1, Ano: 1, _id: 0 })

db.Filmes.find({}, { Nome: 1, Ano: 1, _id: 0 }).sort({ Ano: 1 })

db.Filmes.find({ Nome: "De Volta para o Futuro" }, { Nome: 1, Ano: 1, Duracao: 1, _id: 0 })

db.Filmes.find({ Ano: 1997 }, { Nome: 1, Ano: 1, _id: 0 })

db.Filmes.find({ Ano: { $gt: 2000 } }, { Nome: 1, Ano: 1, _id: 0 })

db.Filmes.find({ Duracao: { $gt: 100, $lt: 150 } }, { Nome: 1, Duracao: 1, _id: 0 }).sort({ Duracao: 1 })

db.Filmes.aggregate([
   { $group: { _id: "$Ano", QuantidadeFilmes: { $sum: 1 } } },
   { $sort: { QuantidadeFilmes: -1 } }
])

db.Atores.find({ Genero: "Masculino" }, { PrimeiroNome: 1, UltimoNome: 1, _id: 0 })

db.Atores.find({ Genero: "Feminino" }, { PrimeiroNome: 1, UltimoNome: 1, _id: 0 }).sort({ PrimeiroNome: 1 })

db.Filmes.aggregate([
   { $lookup: {
       from: "FilmesGenero", 
       localField: "Id", 
       foreignField: "FilmeId", 
       as: "GenerosFilme"
   }},
   { $lookup: {
       from: "Generos", 
       localField: "GenerosFilme.GeneroId", 
       foreignField: "Id", 
       as: "Generos"
   }},
   { $project: { Nome: 1, "Generos.Nome": 1, _id: 0 } }
])
...
db.Filmes.aggregate([
   ...
   { $lookup: {
       from: "Atores", 
       localField: "Elenco.AtorId", 
       foreignField: "Id", 
       as: "Atores"
   }},
   { $project: { Nome: 1, "Atores.PrimeiroNome": 1, "Atores.UltimoNome": 1, "Elenco.Papel": 1, _id: 0 } }
])